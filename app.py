...
def venues():
    ...
    for venue_city, venue_state in locations:
        venue_data_dict = {
            "city": venue_city,
            "state": venue_state,
            "venues": [
                ...
                for v in (
                    Venue.query
                    .filter_by(state=venue_state)
                    .filter_by(city=venue_city)
                    .all()
                )
            ],
        }

        data.append(venue_data_dict)
...
def create_venue_submission():
    ...
    if form.validate_on_submit():
        ...
        for field, message in form.errors.items():
            flash(f"Error in the entry for {field}: {str(message)}")    
